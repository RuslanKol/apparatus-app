...
import { getAccount } from '../account/_selectors';
...
class Legal extends Component {
  ...
  acceptTerms() {
    ...
    this.props.acceptTerms().then(() => this.setState({ loading: false }));
  }
  ...
  isActive = (match, location, element) => {
    return this.props.location.pathname.indexOf(element.path) !== -1;
  };

  componentDidMount() {
    if (this.getUrlParameter('mobile')) {
      document.body.classList.add('mobile-legal-view');
    } else {
      document.body.classList.remove('mobile-legal-view');
    }
  }
  ...
  render() {
    ...
    return this.state.content[activeTab] ? (
      <div className="legal-wrapper pt-0 pt-lg-8">
        <Row>
          <Col lg="3">
                ...
                <Nav tabs className="w-100">
                  {this.state.content.map((element, i) => (
                    <NavItem key={i}>
                      <NavLink
                        className="nav-link"
                        to={`/legal/${element.path}${this.props.location.search}`}
                        isActive={(match, location) => this.isActive(match, location, element)}
                        onClick={this.toggleNavbar}
                      >
                        {element.title}
                      </NavLink>
